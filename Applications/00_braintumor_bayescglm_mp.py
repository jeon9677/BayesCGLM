...
def job(weights, X_input, X_cv, X_train_covariate, X_cv_covariate, Zmat_data, Zmat_cv, idx):
    ...
    model = Model(inputs=mc_model.inputs, outputs=mc_model.layers[8].output)   ##### last layer
    ...
    _model = sm.GLM(Zmat_train, x_const_final, family=sm.families.Poisson())
    _results = _model.fit()
    ...
    inverse_fisher = fisher_inf[17:, 17:]
...
def main():
    ...
    t0 = timer()  # start time
    mc_model = get_model(mc=True, act="softmax")

    h_mc = mc_model.fit([X_train_basis, X_train_covariate], Zmat_train,
                        batch_size=batch_size,
                        epochs=epochs,
                        verbose=1,
                        validation_data=([X_cv_basis, X_cv_covariate], Zmat_cv_y))
    ...
    logging.debug('glm')
    exp_name = f'{n_model}'
    exp_dir = f'model_brain/{exp_name}'
